...
ClientTemplate;
	...
	protected function _set_client_order_by_controller()
	{
		...
		if($this->bAjaxEnabled==TRUE){
			...
			$head_selector = $response_element_id. ((!empty($this->_aTemplates['listElement']))? ' '.$this->_aTemplates['listElement']:'').((!empty($this->_aTemplates['headElement']))? ' '.$this->_aTemplates['headElement']: ''). ' a';
...
//<![CDATA[
		\$(document).ready(function(){
			...
			\$('$head_selector').click(function(e){
				...
				\$('#{$this->sOrderParmName}').val(\$(this).attr('href'));
