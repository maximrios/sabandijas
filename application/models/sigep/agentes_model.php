...
class Agentes_model extends CI_Model {
    ...
    public function numRegs($vcBuscar, $area=1, $cargo=0) {
        ...
        ($cargo == 0)? $cargos = '':$cargos = 'AND idCargo = '.$cargo.' ';
        $estructura = $this->estructura->obtenerUno($area);
        $sql = 'SELECT count(idAgente) AS inCant FROM sigep_view_agentes WHERE lower(busqueda) LIKE ? AND leftArea >= '.$estructura['leftEstructura'].' AND rightArea <= '.$estructura['rightEstructura'].' '.$cargos.';';
