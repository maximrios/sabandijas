...
class Productos_model extends CI_Model {
    ...
    public function numRegs($vcBuscar, $area=1, $cargo=0) {
        $sql = 'SELECT count(idProducto) AS inCant FROM sabandijas_view_productos WHERE lower(nombreProducto) LIKE ? ';
    ...
    public function guardar($aParms) {
        if($aParms[0] == 'NULL' || $aParms[0] == 0) {
            $sql = 'INSERT INTO sabandijas_productos
                    (nombreProducto
                    , descripcionProducto
                    , uriProducto) 
                    VALUES
            ...
            $sql = 'UPDATE sabandijas_productos SET 
    ...
    public function obtenerColores($idProducto=0) {
        $sql = 'SELECT * FROM sabandijas_colores c
        LEFT JOIN sabandijas_productos_colores pc ON c.idColor = pc.idColor
        GROUP BY c.idColor';
